...
async def handle_feature_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "menu_motion":
        context.user_data.update({'feature_type': "motion"})
        keyboard = [
            [InlineKeyboardButton("💃 Tambahkan Gerakan ke Foto", callback_data="mmode_motion")],
            [InlineKeyboardButton("🎭 Tukar Avatar Di Video", callback_data="mmode_swap")],
...
async def handle_motion_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['replace_background'] = True if query.data == "mmode_motion" else False
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            CHOOSING_MOTION_MODE: [
                CallbackQueryHandler(handle_motion_mode, pattern="^(mmode_motion|mmode_swap)$"),
                CallbackQueryHandler(cancel, pattern="^cancel$")
            ],
